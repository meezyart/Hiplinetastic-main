...
module.exports = (image, isLocal = false) => {
    ...
    if (isLocal) {
        ...
        imageWidths.forEach(width => {
            if (imageWidth > width) imageSrcset.push(`${absoluteUrl('')}/assets/images/${imageName}-${width}w.${imageExtension} ${width}w`);
        })

        imageSrcset.push(`${absoluteUrl('')}/assets/images/${imageName}.${imageExtension} ${imageWidth}w`);

    } else {
        // If image is coming from Sanity, generate an image at every size

        imageWidths.forEach(width => imageSrcset.push(`${imageUrl(image, width)} ${width}w`));
    }
